...
const startupSchema = new mongoose.Schema({
  ...
  pitch: {
    ...
    documents: [{
      ...
      type: {
        type: String,
        enum: ['business_plan', 'financial_model', 'market_research', 'prototype', 'other']
      },
      uploadedAt: {
        type: Date,
        default: Date.now
  ...
  funding: {
    ...
    useOfFunds: [{
      category: {
        type: String,
        enum: ['product_development', 'marketing', 'hiring', 'operations', 'legal', 'other']
      },
      percentage: Number,
      amount: Number,
      description: String
  ...
  updates: [{
    ...
    milestone: {
      type: String,
      enum: ['funding', 'product', 'team', 'partnership', 'customer', 'other']
    }
  }],
  ...
  status: {
    type: String,
    enum: ['draft', 'pending_review', 'active', 'funded', 'paused', 'rejected', 'closed'],
    default: 'draft'
  },
  moderationStatus: {
    type: String,
...
startupSchema.virtual('fundingProgress').get(function() {
  return Math.min((this.funding.currentAmount / this.funding.targetAmount) * 100, 100);
});
...
startupSchema.pre('save', function(next) {
  ...
  if (this.funding.currentAmount >= this.funding.targetAmount && this.status === 'active') {
    this.status = 'funded';
...
// Instance methods
startupSchema.methods.addInvestment = function(investorId, amount, transactionId) {
...
startupSchema.methods.updateAIScore = function(scores) {
  ...
  this.aiScore.overall = Object.values(scores).reduce((a, b) => a + b, 0) / Object.keys(scores).length;
...
startupSchema.statics.searchStartups = function(query, filters = {}) {
  ...
  return this.find(searchQuery)
    .sort(query ? { score: { $meta: 'textScore' } } : { 'engagement.views': -1 })
    .populate('founder', 'firstName lastName profilePicture');
};
