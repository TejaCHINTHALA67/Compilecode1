...
router.post(
  ...
  [
    ...
    body('userType')
      .isIn(['entrepreneur', 'investor', 'both'])
      .withMessage('User type must be entrepreneur, investor, or both'),
    body('businessType')
      .isIn(['startup', 'business', 'investor'])
  ...
  async (req, res) => {
    try {
      ...
      res.status(201).json({
        success: true,
        message:
          'Account created successfully! Check your email for account details.',
        data: {
          user: userData,
          token,
...
router.post(
  '/upload-documents',
  auth,
  documentService.getUploadMiddleware().array('documents', 5),
  async (req, res) => {
    try {
      ...
      for (const doc of uploadedDocs) {
        try {
          ...
          console.error(
            'Error sending document confirmation email:',
            emailError,
          );
        }
      }
