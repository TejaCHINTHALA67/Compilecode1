        ...
        data: {
          ...
          roi:
            user.investorProfile.totalInvested > 0
              ? (user.investorProfile.totalReturns /
                  user.investorProfile.totalInvested) *
                100
              : 0,
        },
      });
    ...
    if (targetType === "startup" && analysisType === "investor_analysis") {
      ...
      const investorProfiles = await Promise.all(
        similarInvestments.map(async (investment) => {
          ...
          const investorInvestments = await Investment.find({
          ...
          return {
            investor: {
              ...
              averageInvestment:
                investorInvestments.reduce((sum, inv) => sum + inv.amount, 0) /
                investorInvestments.length,
              preferredSectors: [
                ...new Set(
                  investorInvestments.map((inv) => inv.startup.sector)
                ),
              ],
              preferredStages: [
                ...new Set(investorInvestments.map((inv) => inv.startup.stage)),
              ],
            },
            ...
            reasoning: generateInvestorReasoning(
              startup,
              investorData,
              investorInvestments
            ),
      ...
      const startupProfiles = matchingStartups.map((startup) => {
        const compatibilityScore = calculateStartupCompatibilityScore(
          investor,
          startup,
          investorInvestments
        );
        return {
          startup: {
            ...
            fundingProgress: {
              ...
              percentage:
                (startup.funding.currentAmount / startup.funding.targetAmount) *
                100,
            },
          ...
          compatibilityScore,
          reasoning: generateStartupReasoning(
            investor,
            startup,
            investorInvestments
          ),
      ...
      insights = {
        ...
        summary: generateInvestorInsightsSummary(
          investor,
          topStartups,
          investorInvestments
        ),
...
function calculateCompatibilityScore(startup, investor, investorInvestments) {
  ...
  if (
    ...
    investor.investmentFocus.includes(startup.sector)
  ) {
    score += 40;
  }
  ...
  const avgInvestment =
    ...
    investorInvestments.length;
  const fundingGap =
    startup.funding.targetAmount - startup.funding.currentAmount;
  if (
    avgInvestment > 0 &&
    Math.abs(avgInvestment - fundingGap * 0.1) < avgInvestment * 0.5
  ) {
    score += 20;
  }

  // Activity level (15% weight)
  const recentInvestments = investorInvestments.filter(
    (inv) =>
...
function calculateStartupCompatibilityScore(
  investor,
  startup,
  investorInvestments
) {
  ...
  const fundingProgress =
    (startup.funding.currentAmount / startup.funding.targetAmount) * 100;
...
function generateInvestorReasoning(startup, investor, investments) {
  ...
  if (avgInvestment > 0) {
    reasons.push(
      `Average investment of $${avgInvestment.toLocaleString()} matches funding needs`
    );
  }

  const sectorInvestments = investments.filter(
    (inv) => inv.startup.sector === startup.sector
  );
  if (sectorInvestments.length > 0) {
    reasons.push(
      `Has ${sectorInvestments.length} previous investments in ${startup.sector}`
    );
  }
...
function generateStartupReasoning(investor, startup, investments) {
  ...
  if (fundingProgress > 20) {
    reasons.push(
      `${fundingProgress.toFixed(0)}% funded - showing market validation`
    );
  }
...
function generateStartupInsightsSummary(startup, topInvestors) {
  const avgCompatibility =
    topInvestors.reduce((sum, inv) => sum + inv.compatibilityScore, 0) /
    topInvestors.length;
  const topSectors = [
    ...new Set(topInvestors.flatMap((inv) => inv.investor.preferredSectors)),
  ];

  return {
    overview: `Found ${
      topInvestors.length
    } potential investors with ${avgCompatibility.toFixed(
      0
    )}% average compatibility`,
    keyFindings: [
      `Top investors focus on ${topSectors.slice(0, 3).join(", ")}`,
      `Average investment range: $${Math.min(
        ...topInvestors.map((inv) => inv.investor.averageInvestment)
      ).toLocaleString()} - $${Math.max(
        ...topInvestors.map((inv) => inv.investor.averageInvestment)
      ).toLocaleString()}`,
      `${
        topInvestors.filter((inv) => inv.compatibilityScore > 70).length
...
function generateInvestorInsightsSummary(investor, topStartups, investments) {
  const avgCompatibility =
    topStartups.reduce((sum, startup) => sum + startup.compatibilityScore, 0) /
    topStartups.length;
  const sectors = [
  ...
  return {
    overview: `Analyzed ${
      topStartups.length
    } startups with ${avgCompatibility.toFixed(0)}% average compatibility`,
    keyFindings: [
      `Opportunities across ${sectors.length} sectors: ${sectors
        .slice(0, 3)
        .join(", ")}`,
      `${
        topStartups.filter((s) => s.compatibilityScore > 75).length
      } high-potential matches`,
      `Average funding progress: ${(
        topStartups.reduce(
          (sum, s) => sum + s.startup.fundingProgress.percentage,
          0
        ) / topStartups.length
      ).toFixed(0)}%`,
    ],
